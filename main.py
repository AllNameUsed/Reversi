...
def main():
    ...
    while running:
        draw_board(phantom_piece, illegal_indicator)  # Pass phantom_piece to draw_board
        draw_settings_icon()
        pygame.display.flip()
        clock.tick(60)
                    ...
                    if 0 <= col < COLS and 0 <= row < ROWS:
                        if board[row][col] is None:
                            ...
                            if (game_mode == 'Human vs Human') or (game_mode == 'Human vs CPU' and turn == 'B'):
                                # Optionally, check if move is valid before showing phantom piece
                                # For simplicity, we'll assume any empty spot can show the phantom piece
                ...
                if WIDTH - 50 <= mx <= WIDTH - 10 and 10 <= my <= 50:
                    ...
                    if 0 <= col < COLS and 0 <= row < ROWS:
                        ...
                        if len(all_valid_moves(board=board, color=turn)) == 0:
                            ...
                            if board[row][col] is None:
                                # Optionally, check if move is valid
